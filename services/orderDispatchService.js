...
class OrderDispatchService {
  ...
  async ensureColumns() {
    try {
      await db.query(`ALTER TABLE order_dispatch ADD COLUMN IF NOT EXISTS gst TEXT`);
    } catch (error) {
      Logger.error('Error ensuring order_dispatch columns', error);
    }
  }
  ...
  async createOrder(orderData, products = []) {
    ...
    try {
      ...
      const isRegular = orderTypeLower === 'regular';
      const planned1 = isPreApproval ? await this.getPlannedTimestamp(client, 'Pre Approval') : null;
      const planned2 = isRegular ? await this.getPlannedTimestamp(client, 'Approval of Order') : null;
      ...
      if (products && products.length > 0) {
        for (let i = 0; i < products.length; i++) {
          ...
          const orderRow = new OrderDispatch({
            ...
            // Set initial stage timestamps
            planned_1: planned1,
            planned_2: planned2,
            // For regular orders, set remaining_dispatch_qty to order_quantity immediately
            remaining_dispatch_qty: isRegular ? product.order_quantity : null,
        ...
        const orderRow = new OrderDispatch({
          ...
          planned_2: planned2,
          // For regular orders, set remaining_dispatch_qty to order_quantity immediately
          remaining_dispatch_qty: isRegular ? orderData.order_quantity : null,
          // order_no will be auto-generated by database trigger
        });
        ...
        SELECT (
          CURRENT_TIMESTAMP + COALESCE(
            (
              SELECT stage_time
              FROM process_stages
              WHERE lower(replace(trim(stage_name), ' ', '')) = lower(replace(trim($1), ' ', ''))
  ...
  async getPreApprovalHistory(filters = {}, pagination = {}) {
    try {
      ...
      // Unified Base DO Expression for grouping
      const baseDoExp = `COALESCE(substring(order_no from '^(DO[-\\/](?:\\d{2}-\\d{2}\\/)?\\d+)'), order_no)`;
  ...
  async submitPreApproval(id, data = {}) {
    ...
    try {
      ...
      const actual1 = new Date().toISOString();
      const planned2 = await this.getPlannedTimestamp(client, 'Approval of Order');
